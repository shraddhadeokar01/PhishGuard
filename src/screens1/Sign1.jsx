...
const SignUpScreen = ({ navigation }) => {
  ...
  try {
    // ✅ Step 1: Create Firebase Authentication user (only if email provided)
    if (!emailOrMobile.includes('@')) {
      return showErrorModal('Please use a valid Email ID for signup!');
    }
    ...
    // ✅ Step 2: Save additional details in Realtime Database
    const newUser = {
      name: fullName,
      email: emailOrMobile.trim(),
      contact: '', // mobile alag handle karaycha asel tar user.emailVerified check kar
    };

    await database().ref(`/users/${userId}`).set(newUser);

    // ✅ Step 3: Save in AsyncStorage
    await AsyncStorage.setItem('currentUser', JSON.stringify(newUser));
    await AsyncStorage.setItem('userIdentifier', emailOrMobile.trim());
    ...
    if (error.code === 'auth/email-already-in-use') {
      showErrorModal('This email is already in use!');
    } else if (error.code === 'auth/invalid-email') {
      showErrorModal('Invalid email format!');
    } else if (error.code === 'auth/weak-password') {
      showErrorModal('Password should be at least 6 characters!');
    } else {
      showErrorModal('Error❌ Something went wrong during sign up');
    }
  }
